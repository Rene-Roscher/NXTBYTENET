                ...
                <div class="col-md-6" style="margin-top: 120px;">
                    <div class="card card-shadow">
                        <div class="card-header card-header-transparent py-20 text-center">
                            ...
                            <a href="<?php echo $url; ?>musikbot/<?php echo $server_id; ?>/renew"
                               class="btn btn-primary">Jetzt verlängern</a>
        ...
        <?php } else { ?>
            ...
            <ul class="nav-quick nav-quick-bordered row">
                <li class="nav-item col-md-4 col-4">
                    <a class="nav-link"
                       href="<?php echo $url; ?>musikbot/<?php echo $server_id; ?>" <?php if (empty($_GET['menu'])) {
                ...
                <li class="nav-item col-md-4 col-4">
                    <a class="nav-link"
                       href="<?php echo $url; ?>musikbot/<?php echo $server_id; ?>/settings" <?php if ($_GET['menu'] == 'settings') {
                        echo 'style="color: #1e88e5;"';
                    } ?>>
                ...
                <li class="nav-item col-md-4 col-4">
                    <a class="nav-link"
                       href="<?php echo $url; ?>musikbot/<?php echo $server_id; ?>/radiolist" <?php if ($_GET['menu'] == 'radiolist') {
                        echo 'style="color: #1e88e5;"';
                    } ?>>
                    ...
                    <div class="col-md-3">
                        <div class="card card-shadow">
                            <div class="card-header card-header-transparent py-20 text-center">
                                ...
                                <form method="POST" action="">
                                    <center>
                                        <?php if ($server_status == 'ONLINE') { ?>
                                            <button type="button" disabled class="btn btn-block btn-success"><span>Starten</span>
                                            </button>
                                            <button type="submit" name="sendStop" class="btn btn-block btn-danger">
                                                <span>Stoppen</span></button>
                                            <button type="submit" name="sendRestart" class="btn btn-block btn-warning">
                                                <span>Neustarten</span></button>
                                        <?php } elseif ($server_status == 'OFFLINE') { ?>
                                            <button type="submit" name="sendStart" class="btn btn-block btn-success">
                                                <span>Starten</span></button>
                                            <button type="button" disabled class="btn btn-block btn-danger"><span>Stoppen</span>
                                            </button>
                                            <button type="button" disabled class="btn btn-block btn-warning"><span>Neustarten</span>
                                            </button>
                                        <?php } elseif ($server_status == 'NEED_INSTALL') { ?>
                                            <center>
                                                <button type="submit" name="installServer"
                                                        class="btn btn-block btn-float-lg btn-warning btn-block"><span>Installieren</span>
                                                </button>
                                            </center>
                                        <?php } ?>
                                    </center>
                                ...
                                <br>
                                <a href="<?= $url; ?>musikbot/<?= $server_id; ?>/renew"
                                   class="btn btn-block btn-success"><span>Verlängern</span></a>
                                <br>
                                ...
                                <form method="post">
                                    <select class="form-control" name="teamspeak_ip">
                                        ...
                                        if ($SQLSelectService->rowCount() != 0) {
                                            while ($show = $SQLSelectService->fetch(PDO::FETCH_ASSOC)) { ?>
                                                <option value="<?php echo $show['teamspeak_ip']; ?>:<?php echo $show['teamspeak_port']; ?>"><?php echo $show['teamspeak_ip']; ?>
                                                    :<?php echo $show['teamspeak_port']; ?>
                                                    (ID: <?php echo $show['id']; ?>)
                                                </option>
                                            <?php }
                                        } else {
                                            echo '<option value="">Kein Teamspeak gefunden</option>';
                                            $disabled = true;
                                        } ?>
                                    </select>
                                    <br>
                                    ...
                                    <?php if ($disabled) { ?>
                                        <button class="btn btn-primary btn-block" style="float: right;" disabled>
                                            Speichern
                                        </button>
                                    <?php } else { ?>
                                        <button class="btn btn-primary btn-block" style="float: right;" name="saveCFG">
                                            Speichern
                                        </button>
                                    <?php } ?>
                                </form>
                        ...
                        <div class="card card-shadow">
                            <div class="card-header card-header-transparent py-20 text-center">
                                ...
                                <form method="POST" action="">
                                    ...
                                    <button style="float: right;" type="submit" name="changeStreamSettings"
                                            class="btn btn-primary btn-sm">Speichern
                                    </button>
                                ...
                                <form method="post">
                                    <table class="table mb-0">
                                        ...
                                        <tr>
                                            ...
                                            <th>
                                                <input name="teamspeak_ip"
                                                       value="<?php echo $botInfos['server_addr']; ?>"
                                                       placeholder="127.0.0.1:9987" class="form-control">
                                            </th>
                                        </tr>

                                        <tr>
                                            <th class="text-left">Bot Name</th>
                                            <th><input name="bot_name" value="<?php echo $botInfos['bot_name']; ?>"
                                                       class="form-control"></th>
                                        </tr>

                                        <tr>
                                            <th class="text-left">Server Passwort</th>
                                            <th><input name="ts3_server_password"
                                                       value="<?php echo $botInfos['server_password']; ?>"
                                                       class="form-control"></th>
                                        </tr>

                                        <tr>
                                            <th class="text-left">Channel ID</th>
                                            <th><input name="channel_id"
                                                       value="<?php echo $botInfos['default_channel']; ?>"
                                                       class="form-control"></th>
                                        </tr>

                                        <tr>
                                            <th class="text-left">Channel Passwort</th>
                                            <th><input name="channel_password"
                                                       value="<?php echo $botInfos['channel_password']; ?>"
                                                       class="form-control"></th>
                                        </tr>

                                        </tbody>
                                    </table>
                                    <button type="submit" name="saveCFG" class="btn btn-primary btn-block">Speichern
                                    </button>
                                </form>
                                    ...
                                    if ($SQLSelectServers->rowCount() != 0) {
                                        while ($row = $SQLSelectServers->fetch(PDO::FETCH_ASSOC)) { ?>
                                            <tr>
                                                ...
                                                <td>
                                                    <form method="post">
                                                        <input name="song_url" value="<?php echo $row['id']; ?>"
                                                               hidden="hidden">
                                                        <button class="btn btn-primary btn-sm" type="submit"
                                                                name="changeStreamSettings">Abspielen
                                                        </button>
                                                    </form>
                                                </td>
                                            </tr>
